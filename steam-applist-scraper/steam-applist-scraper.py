# From: https://github.com/mmmmmm44/steam_scraping_api/blob/main/create_app_list/steam-applist-scraper.py
...
def main():
    ...
    latest_apps_dict_ckpt_path, latest_exc_apps_list_ckpt_path, latest_error_apps_list_ckpt_path = check_latest_checkpoints(
        checkpoint_folder, apps_dict_filename_prefix, exc_apps_filename_prefix, error_apps_filename_prefix)

    if latest_apps_dict_ckpt_path:
        apps_dict = load_pickle(latest_apps_dict_ckpt_path)
        print_log('Successfully load apps_dict checkpoint:',
                  latest_apps_dict_ckpt_path)
    ...
    if latest_exc_apps_list_ckpt_path:
        ...
        print_log("Successfully load excluded_apps_list checkpoint:",
                  latest_exc_apps_list_ckpt_path)
        print_log(
            f'Number of apps in excluded_apps_list: {len(excluded_apps_list)}')

    if latest_error_apps_list_ckpt_path:
        error_apps_list = load_pickle(latest_error_apps_list_ckpt_path)
        print_log("Successfully load error_apps_list checkpoint:",
                  latest_error_apps_list_ckpt_path)
    ...
    while len(apps_remaining_deque) > 0:
        ...
        try:
            ...
            if appdetails_req.status_code == 200:
                ...
                print_log(
                    f'Too many requests. Put App ID {appid} back to deque. Sleep for 15 sec')
                ...
                print_log(
                    f'Forbidden to access. Put App ID {appid} back to deque. Sleep for 5 min.')
                apps_remaining_deque.appendleft(appid)
                time.sleep(5 * 60)
                continue
                ...
                print_log(
                    f"Error in App Id: {appid}. Put the app to error apps list.")
            ...
            print_log(
                f"Error in decoding app details request. App id: {appid}")
        ...
        if appdetails['success'] == False:
            ...
            print_log(
                f'No successful response. Add App ID: {appid} to excluded apps list')
            continue
        ...
        if i >= 2500:
            save_checkpoints(checkpoint_folder, apps_dict_filename_prefix, exc_apps_filename_prefix,
                             error_apps_filename_prefix, apps_dict, excluded_apps_list, error_apps_list)
            i = 0
